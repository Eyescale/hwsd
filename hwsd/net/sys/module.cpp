...
NetInfos Module::discover() const
{
    ...
    DWORD datasize = 0;
    if (GetAdaptersAddresses(AF_INET, GAA_FLAG_INCLUDE_PREFIX, 0, 0,
                             &datasize) != ERROR_BUFFER_OVERFLOW)
    {
        return result;
    }

    PIP_ADAPTER_ADDRESSES addresses = (PIP_ADAPTER_ADDRESSES)malloc(datasize);
    if (GetAdaptersAddresses(AF_INET, GAA_FLAG_INCLUDE_PREFIX, 0, addresses,
         ...
         current = current->Next)
    {
        ...
        WideCharToMultiByte(CP_ACP, 0, current->FriendlyName,
                            int(wcslen(current->FriendlyName)), buf, BUFSIZ, 0,
                            0);
        info.name = buf;
    ...
    for (size_t i = 0; i < nInterfaces; ++i)
    {
        ...
        case AF_INET:
            info.inetAddress =
                inet_ntop(item->ifr_addr.sa_family,
                          &((sockaddr_in*)&item->ifr_addr)->sin_addr, address,
                          sizeof(address));
        ...
        case AF_INET6:
            info.inet6Address =
                inet_ntop(item->ifr_addr.sa_family,
                          &((sockaddr_in6*)&item->ifr_addr)->sin6_addr, address,
                          sizeof(address));
    ...
    for (ifaddrs* ifa = ifaddr; ifa; ifa = ifa->ifa_next)
    {
        ...
        case AF_PACKET:
        {
            ...
            {
                mac << std::setw(2) << int(macaddr[i]) << (i < 5 ? ":" : "");
            }
            ...
            default:
                LBWARN << "Unhandled interface type: 0x" << std::hex
                       << unsigned(link->sdl_type) << std::dec << std::endl;
